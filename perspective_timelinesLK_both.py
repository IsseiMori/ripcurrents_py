...
def main(video, outpath, height, window_size, correct_perspective):
	...
	for i_vertex in range(0, len(line_pos) - 1, 2):
		timeline = Timeline(line_pos[i_vertex], line_pos[i_vertex + 1], 40)
		timelines_norm.append(timeline)
	...
	while True:
		...
		if frame_count == 0:
			...
			for timeline in timelines_norm:
				timeline.birth_line()

		for timeline in timelines:
			old_points = timeline.vertices
			new_points, status, error = cv2.calcOpticalFlowPyrLK(old_gray, gray_frame, old_points, None, **lk_params)
			timeline.vertices = change_vertices_step(new_points, old_points, 0.4, 30, False)

		for timeline in timelines_norm:
			old_points = timeline.vertices
			new_points, status, error = cv2.calcOpticalFlowPyrLK(old_gray, gray_frame, old_points, None, **lk_params)
			timeline.vertices = change_vertices_step(new_points, old_points, 0.3, 30, True)
		...
		frame_timelines_norm = resized_frame.copy()
		...
		for i_timeline in range(len(timelines)):
			...
			frame_timelines_norm = timelines_norm[i_timeline].draw_lines(frame_timelines_norm, color_timelines[i_timeline])


		frame_timelines = cv2.putText(frame_timelines, str(frame_count), (30,30), cv2.FONT_HERSHEY_SIMPLEX,  
                   0.8, (255,255,255), 1, cv2.LINE_AA) 
		frame_timelines_norm = cv2.putText(frame_timelines_norm, str(frame_count), (30,30), cv2.FONT_HERSHEY_SIMPLEX,  
                   0.8, (255,255,255), 1, cv2.LINE_AA) 
